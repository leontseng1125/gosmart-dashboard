...
<style>
  ...
  .info-hint{
    ...
    line-height:1;
    margin-left:1px;
    pointer-events:auto; /* 明確聲明，避免日後父層誤設 pointer-events:none 把點擊吃掉 */
  }
  ...
  /* 小網（<=640px）：
     1) popup 寬度用 min() 動態收窄，避免固定 260px 在窄螢幕左右溢出過多。
     2) .jp-row-label 底下的 popup 額外切成 position:fixed —— 因為 .jp-board-wrap
        同時設定了 overflow-x:auto，依 CSS 規範，另一軸即使寫 overflow-y:visible
        也會被隱性轉成 auto，先前的修法實際上沒有效果，popup 只要超出可視高度依然
        會被裁切/擠出畫面。改成 fixed 之後直接以視窗為定位基準，不再受任何父層
        overflow 影響。實際 top/left 座標由下方 JS 的 adjustInfoHintPop() 動態計算。
     3) .jp-row-label .info-hint 增加點擊熱區（負margin抵銷padding，不影響外觀排版），
        方便小螢幕手指點準這個很小的 ⓘ。 */
  @media (max-width:640px){
    .info-hint-pop{ width:min(240px, calc(100vw - 32px)); }

    .jp-row-label .info-hint-pop{
      position:fixed; left:auto; right:auto; top:auto; transform:none; z-index:9999;
    }
    .jp-row-label .info-hint-pop::after{ display:none; } /* fixed定位下尖角無法保證對準ⓘ，先隱藏避免視覺錯位 */

    .jp-row-label .info-hint{ padding:6px 4px; margin:-6px -4px -6px 1px; }
  }
  @media (hover:hover){
  ...
  * { box-sizing: border-box; }
  ...
  /* 這張卡片專屬：角落裝飾線改成跟警示同色的紅色，取代全站 .chart-card 預設的
     rgba(var(--hud-glow)) 綠色角框，只影響這張卡片，不動到其他 .chart-card。 */
  .chart-card.live-anomaly-alert::before,
  .chart-card.live-anomaly-alert::after{
    border-color: rgba(255,56,96,0.7);
  }
  /* ===== 突發異常警示卡片：邊框流光動態效果（Border Beam）=====
     這張卡片的 ::before/::after 已經被上面的紅色轉角裝飾線佔用，所以流光改用
     兩個額外插入的裝飾用 <span>（見HTML，anomaly-beam-wrap／anomaly-beam-mask），
     不動用 pseudo-element，也就不會跟轉角裝飾互相覆蓋。
     做法（純CSS，只用transform:rotate跑動畫，GPU合成、不觸發layout/paint）：
       1) anomaly-beam-wrap：inset:0 且 overflow:hidden 的「窗口」，裁切範圍精準對齊
          卡片本身的圓角，且只裁切這個span自己的子節點，完全不影響卡片本身的
          box-shadow（hover發光）或轉角裝飾線（它們不是這個span的子節點）。
       2) anomaly-beam-spin：塞在窗口裡、遠大於卡片本身（inset:-150%）的巨大
          conic-gradient旋轉色塊——夠大讓旋轉時不用擔心圓角矩形邊角的裁切精準度。
       3) anomaly-beam-mask：疊在流光上方、往內縮1.5px的實色遮罩，圓角跟卡片一致，
          只留外圈1.5px當流光通道，中間完全蓋住看不到流光、不影響文字可讀性。
       4) 卡片內容（圓點、文字）額外設 position:relative + z-index，確保穩定疊在
          流光層最上面，不受HTML中插入順序影響。 */
  .live-anomaly-alert .anomaly-beam-wrap{
    position: absolute;
    inset: 0;
    overflow: hidden;
  ...
  .live-anomaly-alert .anomaly-beam-spin{
    position: absolute;
    inset: -150%;
  ...
  .live-anomaly-alert .anomaly-beam-mask{
    position: absolute;
    inset: 1.5px; /* 露出1.5px當流光通道 */
    border-radius: inherit;
    background: #171a21; /* 貼近 .chart-card 深色底，避免半透明穿透看到流光暈開 */
    pointer-events: none;
    z-index: 0;
  }
  .live-anomaly-alert .ai-alert-dot,
  .live-anomaly-alert .live-anomaly-alert-text{
    position: relative;
    z-index: 1; /* 蓋過流光層，確保文字/圓點永遠在最上面 */
  }
  @media (hover: hover) {
    /* hover時流光轉快一點，呼應既有的hover警戒感（背景變亮＋外發光） */
    .chart-card.live-anomaly-alert:hover .anomaly-beam-spin{ animation-duration: 1.6s; }
  }
  @keyframes anomalyBeamSpin{
  ...
  /* ===== 歷史高頻痛點 Top 5：改以純 HTML/CSS 渲染的雷達光束列表（Beam Fade + 端點呼吸光），
     取代原本 Chart.js 長條圖，沿用全站萊姆綠(#C6F24E)高科技深空雷達視覺語彙 ===== */
  ...
  /* 光束本體：由左至右單向淡出（左側完全透明融入深黑背景，右端收束為實色聚光端點）；
     hover 時圖表本身（光束／端點）維持原樣不加特效，只有文字變色＋顯示明細 tooltip */
  .pain-point-bar {
    position:absolute;
  ...
  /* 端點聚光微標記：白核＋萊姆綠外暈，模擬深空雷達持續追蹤鎖定目標的信號浮標；
     呼吸動畫只掛在這個端點小圓點上，避免整條光束跟著一起脈動 */
  .pain-point-bar::after {
    content:'';
    position:absolute;
    right:0; top:50%;
  ...
  @media (hover: hover) {
    ...
    .pain-point-row:hover .pain-point-label,
    .pain-point-row:hover .pain-point-value {
      color:#C6F24E;
    }
  ...
  .jp-row-label{ flex:0 0 82px; display:flex; align-items:center; justify-content:space-between; font-size:11px; color:rgb(var(--hud-glow)); padding-left:2px; }
  .jp-row-label .jp-arrow{ cursor:pointer; color:rgb(var(--hud-glow)); font-size:12px; transition:transform .15s, color .15s; padding:4px; }
  ...
  .jp-row-label-main{ font-weight:600; color:rgb(var(--hud-glow)); }
  ...
  /* ===== 彩蛋（Easter Eggs）=====
     四個彩蛋互相獨立，視覺沿用既有 --bg / --hud-glow / --hud-grid 語彙，
     不共用 .info-hint-pop，也不掛在既有的全域 document click（避免互相誤關）。 */

  /* 彩蛋1：頭像徽記 + 通訊視窗（HUD Comm Dialog） */
  .scout-avatar {
    display: inline-block;
    cursor: pointer;
    font-size: 20px;
    ...
    filter: drop-shadow(0 0 0 rgba(var(--hud-glow), 0));
  }
  .scout-avatar:hover, .scout-avatar:focus-visible {
    filter: drop-shadow(0 0 8px rgba(var(--hud-glow), 0.85));
  ...
  #scoutCommDialog {
    ...
    background: #11151d;
    border: 1px solid rgba(var(--hud-glow), 0.4);
    border-radius: 6px;
    padding: 14px 16px;
    box-shadow: 0 8px 24px rgba(0,0,0,0.5), 0 0 16px rgba(var(--hud-glow), 0.15);
    opacity: 0;
    transform: translateY(8px);
    pointer-events: none;
    transition: opacity 0.25s ease, transform 0.25s ease;
  }
  #scoutCommDialog.is-visible {
    opacity: 1;
    transform: translateY(0);
    pointer-events: auto;
  }
  #scoutCommDialog .scout-comm-title {
    font-family: "JetBrains Mono", ui-monospace, monospace;
    font-size: 10px;
  ...
  #scoutCommDialog .scout-comm-close {
    cursor: pointer;
    color: var(--muted);
    font-size: 13px;
    line-height: 1;
  ...
  #scoutCommDialog .scout-comm-close:hover { color: rgb(var(--hud-glow)); }
  #scoutCommDialog .scout-comm-body {
    font-family: "JetBrains Mono", ui-monospace, monospace;
    font-size: 12px;
    line-height: 1.6;
    color: var(--text);
  ...
  /* 彩蛋2：戰鬥超頻模式——全螢幕半透明色疊加濾鏡，不動 CSS 變數、不碰圖表本身 */
  #overclockFilterOverlay {
    position: fixed;
    inset: 0;
    z-index: 9990;
    pointer-events: none;
    opacity: 0;
    background: linear-gradient(135deg, rgba(255,56,96,0.18), rgba(255,170,0,0.12));
    mix-blend-mode: hard-light;
  }
  body.overclock-mode #overclockFilterOverlay {
    animation: overclockPulse 2.4s ease-in-out infinite;
  }
  @keyframes overclockPulse { 0%, 100% { opacity: 0.85; } 50% { opacity: 1; } }
  #overclockFlash {
    position: fixed;
    inset: 0;
    z-index: 10000;
  ...
  #overclockFlash.is-flashing { opacity: 1; }

  /* 彩蛋4：深空背景「信號打撈釣魚」——雷達波紋 + 太空漂流物 */
  .ping-wave {
    position: fixed;
    z-index: 5;
    width: 50px;
    ...
    margin-left: -25px;
    margin-top: -25px;
    border: 1px solid rgba(0, 221, 205, 0.55);
    border-radius: 50%;
    transform: scale(0);
    opacity: 0.6;
    pointer-events: none;
  ...
  .drift-item {
    ...
    transition: opacity 1s ease-in-out, transform 2.6s ease-in-out;
  }
  .drift-item .drift-label {
    display: block;
    margin-top: 4px;
    font-size: 9px;
    font-family: "JetBrains Mono", ui-monospace, monospace;
  ...
  #group-blueprint-lab .bp-lab-app::before {
    ...
    inset: 0;
    background-image:
      linear-gradient(rgba(var(--hud-grid), 0.025) 1px, transparent 1px),
      linear-gradient(90deg, rgba(var(--hud-grid), 0.025) 1px, transparent 1px);
  ...
  .bp-lab-modal-type-hint {
    font-size: 10.5px;
    line-height: 1.5;
    color: var(--muted);
    background: rgba(255, 255, 255, 0.03);
    border: 1px solid rgba(255, 255, 255, 0.07);
    border-left: 2px solid rgb(var(--hud-glow));
    border-radius: 4px;
    padding: 6px 8px;
    margin-bottom: 12px;
    min-height: 28px;
...
<body class="hud-booting">
  ...
  <div id="overclockFilterOverlay" aria-hidden="true"></div>
  <div id="overclockFlash" aria-hidden="true"></div>
  ...
  <div id="bpLabAnnoModal" class="bp-lab-modal-overlay" hidden>
    <div class="bp-lab-modal">
      ...
      <!-- 新增：選項小定義提示區塊 -->
      <div id="bpLabAnnoTypeHint" class="bp-lab-modal-type-hint mono"></div>
  ...
  <div style="display:flex; align-items:center; gap:4px; margin-bottom:4px;">
    <span class="scout-avatar" id="scoutAvatar" role="button" tabindex="0" title="領航少尉 阿葛格">🎖️</span>
    <h1 style="margin:0;">領航少尉 阿葛格</h1>
  </div>
  <div id="scoutCommDialog" aria-live="polite">
    <div class="scout-comm-title">[SEC-COM // PRIVATE CHANNEL]<span class="scout-comm-close" id="scoutCommClose" role="button" tabindex="0">✕</span></div>
    <div class="scout-comm-body" id="scoutCommBody"></div>
  </div>
  ...
  <div class="chart-card ai-insight-card" id="aiInsightCard">
    ...
    <div style="display:flex; align-items:center; gap:6px; margin-bottom:16px;">
      <span class="status-pill status-ok mono" id="autoSummaryStatusPill">AUTO_SUMMARY</span>
    ...
    <div class="three-col">
      ...
      <div class="chart-card insight-card-v2 auto-card" id="autoCard2" data-auto-key="pain">
        ...
        <div class="auto-card-chart-wrap">
          <div class="pain-beam-list" id="autoPainList"></div>
        </div>
  ...
  <div id="tab-journeypain">
    ...
    <div class="jp-charts-grid" id="jpChartsGrid">
      <div class="chart-card jp-expandable-card" id="jpQuadInsightCard" data-jp-key="jpquad">
        <h2 style="margin:0 0 2px; font-size:13px;">流程複雜度 × 痛點密集度 <span class="info-hint" tabindex="0">ⓘ<div class="info-hint-pop">X軸＝行為流程子步驟數，Y軸＝負評總則數。右上角＝優先重新設計候選；點擊資料點/下方清單可查看該階段實際負評；點擊卡片其他地方可展開放大圖表</div></span></h2>
      ...
      <div class="chart-card jp-expandable-card" id="jpParetoInsightCard" data-jp-key="jppareto">
        <h2 style="margin:0 0 2px; font-size:13px;">痛點排行榜 <span class="info-hint" tabindex="0">ⓘ<div class="info-hint-pop">依篩選後負評則數排序，點擊可查看實際評論；點擊此區塊可展開查看更多內容</div></span></h2>
  ...
  <script>
    ...
    // ============================================================
    // 彩蛋（Easter Eggs）：全部包在 onHudBootDone() 裡註冊監聽，
    // 確保開場 Loading 播完之前不會生效，避免訪客在光幕期間亂點造成狀態錯亂。
    // ============================================================

    // ----- 彩蛋1：頭像／軍階觸發「少尉的碎碎念」 -----
    onHudBootDone(function () {
      const avatar = document.getElementById('scoutAvatar');
      const dialog = document.getElementById('scoutCommDialog');
      if (!avatar || !dialog) return;
      const bodyEl = document.getElementById('scoutCommBody');
      const closeBtn = document.getElementById('scoutCommClose');
      const SCOUT_DIALOGUES = [
        // 【前線深空與開發辛酸】
        '報告長官：目前 iOS 數據依然由熱心 PM 手動運送回港，請求撥款自動化通訊頻寬！',
        '深空日誌：有位用戶為了藍牙開鎖在停車場繞了 20 分鐘……阿葛格已將其標註在旅程第 7 節點。',
        '早期的 iOS 信號全靠少尉一張張截圖手動拼裝……現在能看到動態點陣，感動度已突破 200%。',
        '警告：艦橋工程師的咖啡存量已降至 15%，建議立即返航補給。',
        '正在深空攔截雜訊……報告，剛打撈到一則因為「找不到還車格」而在路邊靈魂咆哮的微弱頻率。',
        // 【軍階互動與幽默吐槽】
        '連續點擊我並不能讓版本評分回到 5 星，但能讓少尉的心跳頻率超頻 30%。',
        '別再戳少尉的肩章了！再戳……再戳我就要跟總部申請晉升中尉了！',
        '報告長官！目前全艦系統運作正常，除了我的黑眼圈稍微擴散了 2 個像素。',
        '長官，您是來巡檢數據的，還是純粹無聊來戳阿葛格的？',
        '雷達顯示：您已經在艦橋逗留許久，若發現 Bug，請溫柔回報給少尉。',
        '阿葛格的直屬長官為翊凱艦長。',
        // 【戰術導航與功能指引】
        '導航提示：右側的「服務藍圖」藏著 14 個航段的阻力指數，遇到紅光節點記得點開看看。',
        '若想知道最近大家在抱怨什麼，不妨切到 DEEP INSIGHTS，那裡的點陣儀表隨時待命。',
        '報告：如果遇到評分暴跌的版本，記得查看版本健康度，少尉已經把疑似元兇標出來了。',
        // 【彩蛋聯動與極客幽默】
        '聽說在鍵盤敲下古老的星際秘技（↑↑↓↓←→←→BA），這座艦橋會進入高能過載模式……',
        'Console 終端機好像有少尉偷偷留下的徵才密令，工程師長官千萬別按 F12 偷看！',
        '艦橋密令：試著在背景空白處點擊，說不定能從深空打撈出漂流瓶喔！',
        '正在執行例行除錯……偵測到「需求不要再改了」的量子干涉波。'
      ];
      let clickTimes = [];
      let hideTimer = null;
      function hideDialog() {
      ...
      function showDialog() {
        bodyEl.textContent = SCOUT_DIALOGUES[Math.floor(Math.random() * SCOUT_DIALOGUES.length)];
        dialog.classList.add('is-visible');
        clearTimeout(hideTimer);
        hideTimer = setTimeout(hideDialog, 4000);
      }
      ...
      avatar.addEventListener('click', function (e) {
        e.stopPropagation(); // 避免順便冒泡觸發彩蛋4的深空點擊判定
        registerClick();
      });
    ...
    // ----- 彩蛋2：Konami Code「戰鬥過載模式」 -----
    onHudBootDone(function () {
      const KONAMI = ['arrowup', 'arrowup', 'arrowdown', 'arrowdown', 'arrowleft', 'arrowright', 'arrowleft', 'arrowright', 'b', 'a'];
      let buffer = [];
      let savedLabels = null;
      const overlayFlash = document.getElementById('overclockFlash');
      const statusPills = [
        document.getElementById('aiInsightStatus'),
        document.getElementById('autoSummaryStatusPill')
      ].filter(Boolean);

      function triggerFlash() {
        if (!overlayFlash) return;
        overlayFlash.style.transition = 'none';
        overlayFlash.classList.add('is-flashing');
        requestAnimationFrame(function () {
          requestAnimationFrame(function () {
            overlayFlash.style.transition = 'opacity 0.25s ease-out';
            overlayFlash.classList.remove('is-flashing');
          });
        });
      }

      function setOverclock(on) {
        const isOn = document.body.classList.contains('overclock-mode');
        if (on === isOn) return;
        document.body.classList.toggle('overclock-mode', on);
        if (on) {
          savedLabels = statusPills.map(function (el) { return el.textContent; });
          statusPills.forEach(function (el) { el.textContent = 'OVERCLOCK // 200%'; });
          triggerFlash();
        } else {
          if (savedLabels) {
            statusPills.forEach(function (el, i) { el.textContent = savedLabels[i]; });
          }
          savedLabels = null;
        }
      }
      ...
      document.addEventListener('keydown', function (e) {
        ...
        if (e.key === 'Escape') {
          if (document.body.classList.contains('overclock-mode')) setOverclock(false);
          buffer = [];
          return;
        }
        ...
        if (buffer.length === KONAMI.length && buffer.every(function (k, i) { return k === KONAMI[i]; })) {
          buffer = [];
          setOverclock(!document.body.classList.contains('overclock-mode'));
        }
      });
    });
    ...
    onHudBootDone(function () {
      ...
      function spawnDriftItem(x, y) {
        if (Math.random() > 0.15) return; // 約15%機率
        const pick = DRIFT_ITEMS[Math.floor(Math.random() * DRIFT_ITEMS.length)];
        const el = document.createElement('div');
        el.className = 'drift-item';
        el.style.left = x + 'px';
        el.style.top = y + 'px';
        el.innerHTML = pick.icon + (pick.label ? '<span class="drift-label">' + pick.label + '</span>' : '');
        document.body.appendChild(el);
        requestAnimationFrame(function () {
          requestAnimationFrame(function () {
            el.style.opacity = '1';
            el.style.transform = 'translateY(-46px) rotate(24deg)';
          });
        });
        setTimeout(function () {
          el.style.opacity = '0';
      ...
      document.addEventListener('click', function (e) {
        if (e.target.closest('.card, .chart-card, .group-nav, .tabs, .review-drawer, .drawer-overlay, button, input, select, table, a, .info-hint, .scout-avatar, #scoutCommDialog')) return;
        spawnPingWave(e.clientX, e.clientY);
        spawnDriftItem(e.clientX, e.clientY);
      });
    });
    ...
    // Bug fix（手機版滾動時網址列收合/展開，造成「近期熱門負評關鍵字」劇烈閃爍跳動）：
    // 行動瀏覽器（尤其 Chrome/Safari）滾動時網址列收合展開，只會改變 window.innerHeight，
    // 不會改變 innerWidth，但一樣會觸發 window 的 resize 事件。原本 resize handler 沒有
    // 任何防呆，每次都直接呼叫 renderKeywordRows()，而它內部是整段 container.innerHTML
    // 重新賦值——40格 LED 的進場動畫（ledSweepIn，帶 stagger delay）會整批重播一次，
    // 使用者看到的就是「清空→重新掃描點亮」的劇烈閃爍。用兩層防呆解決：
    //   1) lastKeywordRowCount：只有「要顯示的列數」真的改變時才重繪，高度沒變、
    //      算出來還是同樣列數就跳過，不重播動畫。
    //   2) resize handler 加 debounce，且只在「寬度」真的改變時才重新計算版面，
    //      網址列收合展開這種只改高度的 resize 事件直接忽略。
    let lastKeywordRowCount = null;
    ...
    function syncKeywordCardHeight() {
      ...
      if (window.innerWidth <= 1100) {
        card.style.height = '';
        // 窄螢幕固定5列，只有「還沒渲染過」或「上次不是5列」才重繪，
        // 避免每次 resize（含網址列收合展開）都讓LED進場動畫整批重播。
        if (renderKeywordRows && lastKeywordRowCount !== 5) {
          renderKeywordRows(5); // 窄螢幕維持原本固定5列，版面本來就沒問題
          lastKeywordRowCount = 5;
        }
        return;
      }
      ...
      if (targetHeight > 0) {
        ...
        if (renderKeywordRows) {
          ...
          // 只有列數真的變了才重繪，高度微調、算出來還是同樣列數就跳過，不重播動畫。
          if (rowCount !== lastKeywordRowCount) {
            renderKeywordRows(rowCount);
            lastKeywordRowCount = rowCount;
          }
        }
      }
    }
    ...
    // debounce（100ms）+ 只在「寬度」真的改變時才重新計算版面：
    // 手機網址列收合/展開、鍵盤彈出等只會改變 innerHeight 的情況直接忽略，
    // 避免觸發不必要的 forced reflow 與（修好前的）LED 動畫重播。
    let keywordLastViewportWidth = window.innerWidth;
    let keywordResizeDebounceTimer = null;
    window.addEventListener('resize', () => {
      clearTimeout(keywordResizeDebounceTimer);
      keywordResizeDebounceTimer = setTimeout(() => {
        const widthChanged = window.innerWidth !== keywordLastViewportWidth;
        keywordLastViewportWidth = window.innerWidth;
    ...
    (function () {
      ...
      // 「前台互動」「後台/系統」現在跟其他列一樣是 #jpBoard 的子節點，
      // renderBoard() 每次篩選條件變動都會整個重建 #jpBoard，
      // 用這兩個變數記住展開/收合狀態，重建時才不會每次都被重置回收合。
      let jpFrontOpen = false;
      let jpBackOpen = false;
      ...
      function renderBoard() {
        ...
        const actionRow = buildRow('顧客行動', true, null, '色深=負評密度｜大小=則數');
        ...
        // 「前台互動」「後台/系統」現在跟上面各列一樣，是 #jpBoard 底下、.jp-board-wrap
        // 這個橫向捲動容器裡的子節點，所以會共用同一份 min-width:1080px，橫向捲動時
        // 自然跟階段/行動等列同步對齊，不再是外面獨立的一塊。
        const frontRow = buildIndependentRow('jpFrontRowWrap', '前台互動', JOURNEY_FRONT_LABELS_CLIENT, 'jp-front-cell', jpFrontOpen, (open) => { jpFrontOpen = open; });
        board.appendChild(frontRow);
        const backRow = buildIndependentRow('jpBackRowWrap', '後台/系統', JOURNEY_BACK_LABELS_CLIENT, 'jp-back-cell', jpBackOpen, (open) => { jpBackOpen = open; });
        ...
        // 前台/後台現在也計入 #jpBoard 的總高度，要放在這兩列都 appendChild 之後才呼叫，
        // 讓大階段分隔線的高度（drawDividers 內用 board.scrollHeight 量測）算進最新的完整高度。
      ...
      // container 現在改成傳入字串 id（而不是既有 DOM 節點）：
      // 因為這個 row 現在是 #jpBoard 的子節點，每次 renderBoard() 都會被 board.innerHTML=''
      // 整個清空重建，不能再依賴頁面上原本就存在的固定容器，所以改成跟 buildRow() 一樣，
      // 自己 createElement 出一個新的 row 容器（並補回原本的 id，維持外部/測試腳本可能用到的選取方式）。
      // isOpen／onToggle 用來讓展開狀態能被外部（renderBoard 上層的 jpFrontOpen／jpBackOpen）記住，
      // 否則每次篩選條件改變重建 DOM，使用者剛展開的內容就會被重置回收合狀態。
      function buildIndependentRow(id, labelText, valuesMap, cellClass, isOpen, onToggle) {
        const container = document.createElement('div');
        container.className = 'jp-row jp-section-divider';
        container.id = id;

        const label = document.createElement('div');
        label.className = 'jp-row-label';
        label.innerHTML = '<span>' + labelText + '</span><span class="jp-arrow">▾</span>';
        const arrow = label.querySelector('.jp-arrow');
        ...
        arrow.addEventListener('click', () => {
          const nowOpen = !gridWrap.classList.contains('open');
          gridWrap.classList.toggle('open', nowOpen);
          arrow.classList.toggle('open', nowOpen);
          if (onToggle) onToggle(nowOpen);
        });

        // 展開/收合是靠 .jp-collapsible 的 max-height CSS transition 做動畫，
        // 動畫「跑完那一刻」高度才是最終值，所以用 transitionend（而不是 click 當下）
        // 觸發 jpRedrawDividers()，讓大階段的垂直分隔線重新量測 #jpBoard 的總高度並跟著變長/變短。
        gridWrap.addEventListener('transitionend', (e) => {
          if (e.propertyName === 'max-height' && window.jpRedrawDividers) window.jpRedrawDividers();
        });
      ...
      // 「前台互動」「後台/系統」兩列已經改在 renderBoard() 內部建立並 appendChild 進 #jpBoard
      // （見上方 renderBoard 內的 buildIndependentRow 呼叫），這裡不用再各自呼叫一次。
    ...
    document.querySelectorAll('.tab-btn').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        // autoVersionChart 也一併受惠，避免寬高塌陷或圖表跑位（歷史高頻痛點已改為純 CSS 光束列表，不受影響）。
        if (window.Chart && Chart.instances) {
          requestAnimationFrame(() => {
    ...
    // ===== 情緒與類別分析：依「總則數（正面+負面）」降冪排序，「其他」固定放最後一名 =====
    // 每次切換時間範圍（全部/今年/本月/本週）都要重新計算排序，不能只換數值不換順序，
    // 不然長條圖的高低順序會跟標籤對不上。
    function sortedCategoryOrderForRange(range) {
      const stats = dataset.categoryStatsByRange[range];
      const others = dataset.categoryOrder.filter(c => c !== '其他');
      const sorted = others.slice().sort((a, b) => {
        const totalA = stats[a].positive + stats[a].negative;
        const totalB = stats[b].positive + stats[b].negative;
        return totalB - totalA;
      });
      if (dataset.categoryOrder.includes('其他')) sorted.push('其他');
      return sorted;
    }

    const initialCategoryOrder = sortedCategoryOrderForRange('all');
    const categoryChartInstance = new Chart(document.getElementById('categoryChart'), {
      type: 'bar',
      data: {
        labels: initialCategoryOrder,
        datasets: [
          {
            label: '正面',
            data: initialCategoryOrder.map(c => dataset.categoryStatsByRange.all[c].positive),
            backgroundColor: '#c6f24e',
            sentimentKey: 'positive',
          },
          {
            label: '負面',
            data: initialCategoryOrder.map(c => dataset.categoryStatsByRange.all[c].negative),
      ...
      options: {
        ...
        onClick: (evt, elements) => {
          ...
          const el = elements[0];
          // 用當前圖表實際渲染出來的 labels 取值，而不是原始未排序的 dataset.categoryOrder，
          // 避免動態排序後 index 對應到錯的分類、點開錯的評論抽屜。
          const category = categoryChartInstance.data.labels[el.index];
    ...
    document.querySelectorAll('#categoryTimeRangeToggleGroup .toggle-btn').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        const stats = dataset.categoryStatsByRange[range];
        const sortedOrder = sortedCategoryOrderForRange(range);
        categoryChartInstance.data.labels = sortedOrder;
        categoryChartInstance.data.datasets[0].data = sortedOrder.map(c => stats[c].positive);
        categoryChartInstance.data.datasets[1].data = sortedOrder.map(c => stats[c].negative);
    ...
    // ===== 長條圖漸層＋柔光發光共用工具（用戶旅程階段檢視／意圖分佈／字詞頻率排行三張圖共用）=====
    // 由透明到實色的線性漸層，模擬紐莫菲柔和發光質感；chartArea 在圖表第一次版面配置完成前是
    // undefined，這裡做防護直接回傳純色，避免初次繪製時噴 undefined 錯誤。
    function getBarGradient(ctx, chartArea, colorRgb, isHorizontal, stops) {
      if (!chartArea) return 'rgba(' + colorRgb + ',' + stops[2] + ')';
      const gradient = isHorizontal
        ? ctx.createLinearGradient(chartArea.left, 0, chartArea.right, 0)
        : ctx.createLinearGradient(0, chartArea.bottom, 0, chartArea.top);
      gradient.addColorStop(0, 'rgba(' + colorRgb + ',' + stops[0] + ')');
      gradient.addColorStop(0.5, 'rgba(' + colorRgb + ',' + stops[1] + ')');
      gradient.addColorStop(1, 'rgba(' + colorRgb + ',' + stops[2] + ')');
      return gradient;
    }
    // 柔光 plugin：在 Chart.js 真正畫柱子「之前」，先手動依每根柱子自己的像素位置畫一層
    // 半透明色塊＋陰影模糊，當作墊在柱子底下的光暈；每根柱子的陰影色可以逐一指定
    // （dataset.glowColors 陣列，依 index 對應），沒有陣列就退回 dataset.glowColor 整個
    // dataset 共用一色，兩者都沒設定才用中性深色。用手動畫色塊而不是整個 dataset 共用一次
    // shadowColor，是因為 Chart.js 沒有「每根柱子各自陰影色」的內建掛鉤，必須自己算出每根
    // 柱子的像素幾何（垂直/水平兩種方向分開處理）才能做到逐柱不同色。
    function resolveGlowColor(ds, index) {
      if (Array.isArray(ds.glowColors)) return ds.glowColors[index] || 'rgba(0,0,0,0.35)';
      return ds.glowColor || 'rgba(0,0,0,0.35)';
    }
    const barGlowPlugin = {
      id: 'barGlow',
        ...
        chart.data.datasets.forEach((ds, dsIndex) => {
          ...
          meta.data.forEach((el, i) => {
            ...
            if (horizontal) {
              ...
              y = el.y - el.height / 2;
              w = Math.abs(el.x - el.base);
              h = el.height;
            } else {
              x = el.x - el.width / 2;
              y = Math.min(el.y, el.base);
              w = el.width;
              h = Math.abs(el.base - el.y);
            }
            if (w <= 0 || h <= 0) return;
            const glowColor = resolveGlowColor(ds, i);
            ctx.save();
    ...
    // ===== 情緒分析 tab：意圖分佈 =====
    const intentColorsRgb = ['255,107,107', '0,221,205', '198,242,78', '91,98,114'];
      ...
      data: {
        ...
        datasets: [{
          ...
          backgroundColor: (context) => {
            ...
            const rgb = intentColorsRgb[context.dataIndex] || '198,242,78';
            return getBarGradient(chart.ctx, chart.chartArea, rgb, true, [0.05, 0.4, 1]);
          },
          borderRadius: { topRight: 4, bottomRight: 4 },
          glowColors: intentColorsRgb.map(rgb => 'rgba(' + rgb + ',0.3)'),
        }],
      },
      options: {
          responsive: true,
    ...
    document.querySelectorAll('#stageTimeRangeToggleGroup .toggle-btn').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        stageChartInstance.data.datasets[0].data = dataset.stageOrder.map(s => stats[s].negative);
        stageChartInstance.data.datasets[1].data = dataset.stageOrder.map(s => stats[s].positive);
        ...
        wordChartInstance = new Chart(canvas, {
          ...
          data: {
            ...
            datasets: [{
              ...
              backgroundColor: (context) => {
                const chart = context.chart;
                return getBarGradient(chart.ctx, chart.chartArea, '198,242,78', true, [0.05, 0.4, 1]);
              },
              borderRadius: { topRight: 4, bottomRight: 4 },
              glowColor: 'rgba(198,242,78,0.3)',
            }],
    ...
    (function renderAutoSummary() {
      ...
      function setupAutoCardExpand(cardId, detailId, getChart) {
        ...
        card.addEventListener('click', (e) => {
          ...
          if (e.target.closest('canvas')) return;
          if (e.target.closest('.pain-beam-list')) return;
      ...
      if (versionCanvas && window.Chart) {
        ...
        autoVersionChart = new Chart(versionCanvas, {
          ...
          options: {
            ...
            layout: {
              padding: { right: 24, top: 4 }, // 24px：確保最新版本呼吸光暈全開時（最大半徑約18px＋緩衝）不會被canvas右邊緣裁到，
                                               // 不能只靠 x軸 offset 留的半格空間，版本數一多那個空間會被壓縮到不夠用
            },
        ...
        function drawLatestVersionGlow(timestamp) {
          ...
          if (rect.width > 0 && rect.height > 0) {
            ...
            if (point && typeof point.x === 'number') {
              ...
              const outerRadius = 7 + wave * 11; // 7px → 18px：擴散範圍縮減為原本(14→36)的一半
              const outerAlpha = 0.28 + wave * 0.72; // 0.28 → 1.0：拉大明暗對比，低谷要明顯淡、高峰要明顯亮
              const innerRadius = 3 + wave * 5; // 3px → 8px：白核柔光範圍縮減為原本(6→16)的一半
      ...
      // 純 HTML/CSS 雷達光束列表：以線段長度呈現負評則數（原 Chart.js 長條圖已移除，
      // 改用 CSS 漸層光束＋端點呼吸光；hover 只做文字變色，圖表本身不加特效，
      // 明細改用 data-tooltip + CSS attr() 做的懸浮 tooltip 呈現，詳見上方 .pain-point-* 樣式）
      const painListEl = document.getElementById('autoPainList');
      if (painListEl) {
        const maxPainCount = Math.max(1, ...painPoints5.map(p => p.negativeCount));
        painListEl.innerHTML = painPoints5.length
          ? painPoints5.map((p) => {
              ...
              return (
                '<div class="pain-point-row" data-category="' + p.category + '" data-tooltip="' + tooltip + '">' +
                  ...
                  '<div class="pain-point-track"><div class="pain-point-bar" style="width:' + pct + '%;"></div></div>' +
                  '<div class="pain-point-value">' + p.negativeCount + ' 則</div>' +
                '</div>'
              );
            }).join('')
          : '<div class="note">目前沒有偵測到明顯的高頻痛點</div>';

        painListEl.querySelectorAll('.pain-point-row').forEach((row) => {
          row.addEventListener('click', () => {
            const p = painPoints5.find(item => item.category === row.dataset.category);
            if (!p) return;
            const matched = dataset.allReviewsFlat.filter(r => r.categories.includes(p.category) && r.sentiment === 'negative');
            openReviewDrawer(p.category + ' 的歷史負評', '共 ' + matched.length + ' 則', matched);
          });
      ...
      setupAutoCardExpand('autoCard2', 'autoCard2Detail');
    ...
    (function renderAIInsight() {
      ...
      function goToAIInsightFromLiveBanner() {
        ...
        // 避免 autoVersionChart 沿用切走前的舊尺寸而跑位或塌陷（歷史高頻痛點已改為純 CSS 光束列表，不受影響）。
        requestAnimationFrame(() => {
          if (window.Chart && Chart.instances) {
            Object.values(Chart.instances).forEach((c) => { try { c.resize(); } catch (e) {} });
          }
    ...
    function adjustInfoHintPop(hintEl) {
      ...
      // 小網 + .jp-row-label：popup 目前是 position:fixed（見上方 CSS media query），
      // 直接拿 hintEl 在「視窗」中的座標算 top/left 寫進 style，不吃 .info-hint 的
      // relative 定位，也完全不受 .jp-board-wrap 內部橫向捲動位移影響（fixed 是以
      // 視窗、不是以捲動內容為基準），藉此徹底避開 overflow-x:auto 造成的裁切問題。
      if (isLeftAligned && isSmallScreen) {
        const margin = 12;
        const hintRect = hintEl.getBoundingClientRect();
        pop.style.left = '0px';
        pop.style.top = (hintRect.bottom + 6) + 'px';
        requestAnimationFrame(() => {
          let rect = pop.getBoundingClientRect();
          if (rect.bottom > window.innerHeight - margin) {
            // 貼近視窗底部：翻到 ⓘ 上方展開
            pop.style.top = (hintRect.top - rect.height - 6) + 'px';
...
(function () {
  ...
  // 標籤選項小定義：openAnnoModal / bindAnnoTypeButtons 切換類型時同步更新 #bpLabAnnoTypeHint
  var TYPE_DESCRIPTIONS = {
    'F': '<span style="color:var(--neg);">✕ F 失敗風險</span>：功能故障、閃退或流程中斷，需工程修復。',
    'D': '<span style="color:var(--hypo);">D 決策疑慮</span>：規則不透明、判定門檻不清，需文案指引優化。',
    'W': '<span style="color:var(--wait);">W 預期等待</span>：流程需耗時但缺乏進度指示，需補足狀態回饋。',
    'idea': '<span style="color:var(--idea);">💡 機會點</span>：可創造體驗亮點、差異化優勢或提升轉換率的想法。',
    'hypo': '<span style="color:var(--hypo);">❓ 假設</span>：尚未經數據驗證、易用性測試或待確認可行性的推論。'
  };

  function updateAnnoTypeHint(type) {
    var hintEl = document.getElementById('bpLabAnnoTypeHint');
    if (!hintEl) return;
    if (type && TYPE_DESCRIPTIONS[type]) {
      hintEl.innerHTML = TYPE_DESCRIPTIONS[type];
      hintEl.style.display = 'block';
    } else {
      hintEl.innerHTML = '請點選上方標籤類型以檢視定義';
    }
  }
  ...
  function bindAnnoTypeButtons() {
    ...
    Array.prototype.forEach.call(typesContainer.querySelectorAll('.bp-lab-type-btn'), function (b) {
      b.addEventListener('click', function () {
        ...
        updateAnnoTypeHint(currentAnnoType);
      });
    });
  }
